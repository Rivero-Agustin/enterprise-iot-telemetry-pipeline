...
// @CUSTOM_PATCH: inicializacion de la función de callback para datos customizados
// Fecha: 16-Jul-2026 | Autor: Agustin Rivero
void (*DW1000RangingClass::_handleCustomData)(const char*) = 0;
// @END_CUSTOM_PATCH
...
void DW1000RangingClass::loop() {
	...
	if(_receivedAck) {
		...
		// @CUSTOM_PATCH: Original comentado para no romper el dato
		// DW1000.getData(data, LEN_DATA);

		// @CUSTOM_PATCH: Interceptor del byte `0x80` para derivarlo a un callback
		// Fecha: 16-Jul-2026 | Autor: Agustin Rivero
		// Motivo: La librería original no soporta envío de payloads customizados.
		byte rxData[128];
		uint16_t len = DW1000.getDataLength();
		DW1000.getData(rxData, len);

		// Si el primer byte es 0x80, es un comando, no una distancia
		if (rxData[0] == 0x80) {
			char* textoRecibido = (char*)(rxData + 1);
			if (_handleCustomData != 0) {
				_handleCustomData(textoRecibido);
			}
			return;
		}
		// Si no es texto, copiamos la data al buffer normal y seguimos calculando distancia
		memcpy(data, rxData, LEN_DATA);
		// @END_CUSTOM_PATCH
...
// @CUSTOM_PATCH: Manejador de callback para datos customizados
// Fecha: 16-Jul-2026 | Autor: Agustin Rivero
	void DW1000RangingClass::attachCustomDataReceived(void (*handleCustomData)(const char*)) {
    _handleCustomData = handleCustomData;
}
// @END_CUSTOM_PATCH
...
// @CUSTOM_PATCH: Método para transmitir datos customizados por UWB
// Fecha: 16-Jul-2026 | Autor: Agustin Rivero
void DW1000RangingClass::transmitCustomData(const char* text) {
    DW1000.newTransmit();
    DW1000.setDefaults();
    byte payload[128];
    payload[0] = 0x80; // 0x80 = Nuestro código para "ESTO ES TEXTO"
    
    // Copiamos el texto justo después de nuestro código secreto
    memcpy(payload + 1, text, strlen(text) + 1);
    
    DW1000.setData(payload, strlen(text) + 2);
    DW1000.startTransmit();
}
// @END_CUSTOM_PATCH
