...
public:
	...
	// @CUSTOM_PATCH: Funciones publicas para transmitir datos customizados por UWB
    // Fecha: 16-Jul-2026 | Autor: Agustin Rivero
    // Motivo: La librería original no soporta envío de payloads customizados.
	static void transmitCustomData(const char* text);
	static void attachCustomDataReceived(void (*handleCustomData)(const char*));
	// @END_CUSTOM_PATCH
...
private:
	...
	// @CUSTOM_PATCH: Declaración del puntero privado para la función de callback de datos customizados
    // Fecha: 16-Jul-2026 | Autor: Agustin Rivero
	static void (*_handleCustomData)(const char*);
	// @END_CUSTOM_PATCH
